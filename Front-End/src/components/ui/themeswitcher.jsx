...
export default function ThemeSwitcher({ position = 'static', top = '', right = '' }) {
  ...
  useEffect(() => {
    if (open) {
      document.body.classList.add('theme-modal-blur');
    } else {
      document.body.classList.remove('theme-modal-blur');
    }
    return () => {
      document.body.classList.remove('theme-modal-blur');
    };
  ...
  return (
    <div className={`${position} ${top} ${right}`}>
      ...
      {open && (
        <>
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[1000]" onClick={() => setOpen(false)} />
